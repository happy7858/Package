...
/**
 * eg:  将数据写入SVMapping,再转换为字符串
 	public String GetUserGameData() {
	  	SVCMapping map = new SVCMapping();
		SStringReadWrite rw = new SStringReadWrite(map);
		map.set("MaxStage", m_nMaxStage);
		map.set("MaxScore", m_nRankScore);
		map.set("Title", m_nCurrTitle);
		return rw.write();
	}

 * eg:  将字符串转换为SVCMapping，再从中读取数据
	public void LoadDataFromSVCMapping(String data) {  
		SVCMapping map = new SVCMapping();
		SStringReadWrite rw = new SStringReadWrite(map);
		rw.read(data);
	
		m_lifenum = map.query("Life");// 增加生命购买情况
		m_nCurrStage = map.query("Stage");// 当前游戏的关卡-hcl-[2011-10-22]
		m_nCurrScore = map.query("Score");
	}
*/

public class SStringReadWrite implements SIReadWrite {
	protected SVCMapping m_map;

	public SStringReadWrite(SVCMapping m) {
		m_map = m;
	}
